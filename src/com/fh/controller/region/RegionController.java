package com.fh.controller.region;
...
import com.fh.service.region.RegionService;
...
public class RegionController extends BaseController {
	...
	String menuUrl = "region/list.do"; //菜单地址(权限用)
	...
	public ModelAndView goAdd(){
		...
		pd = this.getPageData();
		try {
			mv.setViewName("region/aggztj_dq_3/aggztj_dq_3_edit");
	...
	public ModelAndView goEdit(){
		...
		pd = this.getPageData();
		try {
			pd = regionService.findById(pd);	//根据ID读取
			mv.setViewName("region/aggztj_dq_3/aggztj_dq_3_edit");
	...
	public Object deleteAll() {
		...
		try {
			...
			if(null != DATA_IDS && !"".equals(DATA_IDS)){
				...
				regionService.deleteAll(ArrayDATA_IDS);
				pd.put("msg", "ok");
			}else{
				pd.put("msg", "no");
			}
	...
	public ModelAndView exportExcel(){
		...
		try{
			...
			List<PageData> varOList = regionService.listAll(pd);
			List<PageData> varList = new ArrayList<PageData>();
			for(int i=0;i<varOList.size();i++){
				PageData vpd = new PageData();
				vpd.put("var1", varOList.get(i).getString("DWMC"));	//1
				vpd.put("var2", varOList.get(i).getString("SJDQ_JCJS"));	//2
				varList.add(vpd);
			}
