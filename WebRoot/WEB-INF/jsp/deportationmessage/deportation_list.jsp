...
<div id="page-content" class="clearfix">
  ...
  <div class="row-fluid">
	...
	<div class="row-fluid">
			...
			<!-- 检索  -->
			<form action="deportation/list.do" method="post" name="Form" id="Form">
			<table>
				<tr>
					<td>
			...
			<table id="table_report" class="table table-striped table-bordered table-hover">
				...
				<thead>
					<tr>
						...
						<th class="center">送案（次）</th>
						<th class="center">文书移送（次）</th>
						...
						<c:forEach items="${varList}" var="var" varStatus="vs">
							<tr>
								...
								<td class='center' style="width: 30px;">${vs.index+1}</td> --%>
										<td>
											<a href="<%=basePath%>deportation/listbyUnitname?DWBM=${var.DWBM}&DWMC=${var.DWMC}">${var.DWMC}</a>
										</td>
										<td>
											<a href="<%=basePath%>deportation/listbySendthecase?DWBM=${var.DWBM}">${var.SAJL_JCJS}</a>
										</td>
										<td>
											<a href="<%=basePath%>deportation/listbyDocumenttransfer?DWBM=${var.DWBM}">${var.WSYSJL_JCJS}</a>
										</td>
	...
	<!-- PAGE CONTENT ENDS HERE -->
		...
		function add(){
			 ...
			 diag.Drag=true;
			 diag.Title ="新增";
			 diag.URL = '<%=basePath%>deportation/goAdd.do';
			 diag.Width = 450;
			 diag.Height = 355;
		...
		function del(Id){
			bootbox.confirm("确定要删除吗?", function(result) {
				if(result) {
					top.jzts();
					var url = "<%=basePath%>deportation/delete.do?AGGZTJ_DQ_3_ID="+Id+"&tm="+new Date().getTime();
		...
		function edit(Id){
			 ...
			 diag.Drag=true;
			 diag.Title ="编辑";
			 diag.URL = '<%=basePath%>deportation/goEdit.do?AGGZTJ_DQ_3_ID='+Id;
			 diag.Width = 450;
			 diag.Height = 355;
			 diag.CancelEvent = function(){ //关闭事件
				 if(diag.innerFrame.contentWindow.document.getElementById('zhongxin').style.display == 'none'){
				...
				if(result) {
					...
					if(str==''){
						...
						if(msg == '确定要删除选中的数据吗?'){
							...
							$.ajax({
								type: "POST",
								url: '<%=basePath%>deportation/deleteAll.do?tm='+new Date().getTime(),
						    	data: {DATA_IDS:str},
								dataType:'json',
		...
		//导出excel
		function toExcel(){
			window.location.href='<%=basePath%>deportation/excel.do';
		}
		</script>
